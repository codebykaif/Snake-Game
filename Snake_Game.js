var Snake = function (){
        ...
        var game = {
            reset: function () {
                ...
                //for(var i=0; i<trail; i++) trail.push({ x:player.x, y:player.y});
                },
                action: {
                    up: function(){
                    ...
                    left: function(){
                        if (lastAction != ActionEnum.right){velocity.x = -1; velocity.y = 0;}
                    },
                    right: function() {
                        if(lastAction != ActionEnum.left){velocity.x = 1; velocity.y =0;}
                    }
                },
                RandomFruit: function () {
                    if(walls){
                        ...
                        fruit.x = Math.floor(Math.random() * tileCount); fruit.y = Math.floor(Math.random()* tileCount);
                    }
                },
                log: function() {
                    console.log('==============');
                ...
                loop: function() {
                    ...
                    if(velocity.x == 0 && velocity.y == -1) latsAction = ActionEnum.up;
                    if(velocity.x == 0 && velocity.y ==1) lastAction = ActionEnum.down;
                    if(velocity.x == -1 && velocity.y == 0) latsAction = ActionEnum.left;
                    ...
                    if(!stopped){
                        ctx.fillStyle = 'rgba(200,200,200,0.2'; ctx.font = "small-caps 14px Helvetica"; ctx.fillText("(space) pause", 24,374); 
                    }
                    ctx.fillStyle = 'green';
                    for(var i=0; i<trail.length-1; i++){
                        ctx.fillRect(trail[i].x *gridSize+1, trail[i].y*gridSize+1, gridSize-2, gridSize-2);
                        // console.debug( i+ ' => player:' + player.x, player.y,+ ', trail:' + trail[i].x, trail[i].y);
                        if(!stopped && trail[i].x == player.x && trail[i].y){
                            game.reset();
                        }
                        ctx.fillStyle = 'lime';
                    }
                    ...
                    if(player.x == fruit.x && player.y == fruit.y){
                        if(!fixedTail) tail++; points++;
                        if(points > pointMax) pointMax = points; reward = 1;
                        game.RandomFruit();
                    ...
                    if(stopped){
                        ...
                        ctx.fillText("Top:" + pointMax, 292, 60);
